...
const auth = {
    createConsoleData(deviceId, serialNumber, deviceType, platformId, systemVersion, language, region, country, cert) {
        if (new nn_certs.NintendoCertificate(cert).valid == false) {
    ...
    getConsoleDataByDeviceId: function (devId) {
        knex.select('id')
            .from('devices')
            .where({
                deviceId: devId
    ...
    checkAuth(req, res, next) {
        ...
        //TODO: Validate cert if provided and reject request otherwise
        ...
        if (cert && new nn_certs.NintendoCertificate(cert).valid == false) {
        ...
        const consoleData = this.getConsoleDataBySerial(serial);
        if(consoleData != false){
            if(consoleData.deviceId != device_id){
                logger.error("[auth]: Authentication was attempted but headers did not match DB!");
